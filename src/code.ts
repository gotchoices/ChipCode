...
export function validateCode(salt: string, options: CodeOptions) {
 ...
 */
export function generateCode(options: CodeOptions) {
    ...
    do {
        ...
        ++tries;
    } while (!validateCode(candidate, options));
    return candidate;
}
 ...
 */
export function makeNonce(identifier: string, code: string) {
